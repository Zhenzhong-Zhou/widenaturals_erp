...
import {
  ...
  selectWarehouseInventoryAdjustQuantitySuccess,
...
import { resetWarehouseInventoryAdjustQuantity } from '@features/warehouseInventory/state/warehouseInventoryAdjustQuantitySlice';
...
const useWarehouseInventoryAdjustQuantity = () => {
  ...
  const adjustResponse = useAppSelector(
    selectWarehouseInventoryAdjustQuantityResponse
  );
  ...
  const adjustQuantities = async (
    ...
    return dispatch(
      adjustWarehouseInventoryQuantitiesThunk({ warehouseId, payload })
    );
  };
