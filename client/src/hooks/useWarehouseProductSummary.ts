...
const useWarehouseProductSummary = (
  warehouseId: string,
  ...
  const [productSummaryPage, setProductSummaryPage] =
    useState<number>(initialPage);
  const [productSummaryLimit, setProductSummaryLimit] =
    useState<number>(initialLimit);

  // Redux state
  const productSummary = useAppSelector(selectWarehouseProductSummary);
  const productSummaryPagination = useAppSelector(
    selectWarehouseProductPagination
  );
