...
export interface AllocateInventoryDialogProps extends Omit<CustomDialogProps, 'title'> {
  orderId: string;
  category: string;
}
...
const AllocateInventoryDialog: FC<AllocateInventoryDialogProps> = ({
  ...
  useEffect(() => {
    if (!warehouseId && warehouseLookupOptions.length > 0) {
      const defaultWarehouse = warehouseLookupOptions.find(
        (w) => w.label === 'WIDE Naturals Inc. (Head Office Warehouse - distribution_center)'
      );
  ...
  return (
    <CustomDialog
      ...
      showCancelButton
      confirmButtonText={canAllocate ? (isAllocating ? 'Allocating...' : 'Allocate') : undefined}
    ...
    >
      ...
      {allocationError && (
        <ErrorMessage message={allocationError} />
      )}
