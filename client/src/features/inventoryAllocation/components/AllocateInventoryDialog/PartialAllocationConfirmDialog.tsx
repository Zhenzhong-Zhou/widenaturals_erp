...
const PartialAllocationConfirmDialog: FC<
  PartialAllocationConfirmDialogProps
> = ({ open, items, onCancel, onConfirm }) => {
  return (
    <CustomDialog
      open={open}
      onClose={onCancel}
          ...
          <TableBody>
            {items.map((item, index) => (
              <TableRow key={index}>
                ...
                <TableCell align="right">{item.allocatedQuantity}</TableCell>

                <TableCell align="right">{item.missingQuantity}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
