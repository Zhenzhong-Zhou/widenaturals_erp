...
const OrderPage: FC = () => {
  const [selectedOrderType, setSelectedOrderType] = useState<{ id: string; name: string } | null>(null);
  ...
  return (
    <Box sx={{ p: 3, maxWidth: "600px", mx: "auto" }}>
      ...
      <OrderTypesDropdown value={selectedOrderType?.id || null} onChange={handleOrderTypeChange} />
      ...
      <CustomModal open={isModalOpen} onClose={() => setModalOpen(false)}>
        <Box sx={{ p: 4, bgcolor: "white", borderRadius: 2, maxWidth: 500, mx: "auto", mt: 10 }}>
          <Typography variant="h6" gutterBottom>
            Create {selectedOrderType?.name?.includes("Order") ? selectedOrderType?.name : `${selectedOrderType?.name} Order`}
          </Typography>
