...
const AdjustmentReportTable: React.FC<AdjustmentReportTableProps> = ({ data, pagination, filters, setFilters, fetchReport }) => {
  ...
  return (
    <CustomTable
      ...
      onPageChange={(newPage) => {
        setFilters((prev: AdjustmentReportParams) => ({ ...prev, page: newPage + 1 }));
        fetchReport({ ...filters, page: newPage + 1 });
      }}
      onRowsPerPageChange={(newLimit) => {
        setFilters((prev: AdjustmentReportParams) => ({ ...prev, limit: newLimit, page: 1 })); // Reset to first page
