import { type FC, useCallback, useEffect, useState, Suspense, useMemo, lazy } from 'react';
...
import type { OutboundFulfillmentFilters, OutboundFulfillmentSortKey } from '@features/outboundFulfillment/state';
...
const OutboundFulfillmentsListPage: FC = () => {
  ...
  const [filters, setFilters] = useState<OutboundFulfillmentFilters>({} as OutboundFulfillmentFilters);
  ...
  return (
    <Box sx={{ px: 4, py: 3 }}>
      ...
      <Card sx={{ p: 3, mb: 4, borderRadius: 2, minHeight: 200 }}>
        {outboundFulfillmentsLoading && <Loading variant="dotted" message="Loading fulfillments..." />}
        {outboundFulfillmentsError && <CustomTypography color="error">{outboundFulfillmentsError}</CustomTypography>}
        {!outboundFulfillmentsLoading && !outboundFulfillmentsError && outboundFulfillments.length === 0 && (
