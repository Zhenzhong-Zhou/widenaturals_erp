...
const BulkInsertInventoryModal: FC<{ warehouseId: string; onSubmit: (data: Record<string, any>[]) => void; mode: "create" | "edit" | "adjust" }> = ({ warehouseId, onSubmit, mode }) => {
  ...
  const getValidationRules = (mode: "create" | "edit" | "adjust") => (watch: (name: string) => any) => ({
    ...
    product_id: (value: string) => {
      if (watch("items")?.some((item: any) => item.type === "product" && (!value || value.trim() === ""))) {
        return "Product ID is required for products";
      }
      return undefined;
    },
    lot_number: (value: string) => {
      if (watch("items")?.some((item: any) => item.type && (!value || value.trim() === ""))) {
              ...
              fields={[
                ...
                },
                {
                  ...
                  component: ({ value, onChange }) => (
                    <InventoryDropdown label="Product" options={productOptions} value={value} onChange={onChange} searchable />
                  ),
                },
                {
                  id: "manufacture_date",
