...
const WarehouseInventoryDetailHeader: FC<WarehouseDetailHeaderProps> = ({ warehouseDetails, loading, refetch }) => {
  ...
  return (
    <Box sx={{ mb: 3, textAlign: "left", p: 2, bgcolor: "background.default", borderRadius: 2 }}>
      ...
      {/* Storage Capacity */}
      <Typography variant="body1" color="textSecondary">
        Storage Capacity: {warehouseDetails.storageCapacity.toLocaleString()} units
      </Typography>
      ...
      {warehouseDetails.status?.name && (
        <Typography
          variant="body1"
          color={warehouseDetails.status.name.toLowerCase() === "active" ? "success.main" : "error.main"}
        >
      ...
      {warehouseDetails.metadata && (
        <Typography variant="body2" color="textSecondary" sx={{ mt: 2 }}>
          Created by: {warehouseDetails.metadata.createdBy ?? "Unknown"} on {formatDate(warehouseDetails.metadata.createdAt ?? null)}
          <br />
          Last updated by: {warehouseDetails.metadata.updatedBy ?? "Unknown"} on {formatDate(warehouseDetails.metadata.updatedAt ?? null)}
        </Typography>
      )}
