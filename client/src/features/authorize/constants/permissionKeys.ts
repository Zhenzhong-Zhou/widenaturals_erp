/**
 * Centralized permission key constants mirrored from the server.
 *
 * Mirrors `server/src/utils/constants/domain/permission-keys.js`. Keep these
 * two files in sync — any change here must have a matching change on the
 * server, and vice versa.
 *
 * Usage: reference as `PERMISSION_KEYS.DOMAIN.ACTION`. Never destructure
 * domain objects out of this constant.
 *
 * ACL-layer permissions live in their own domain constants files, not here.
 */
...
export const PERMISSION_KEYS = {
  ...
  INVENTORY_ALLOCATION: {
    ...
    CONFIRM: 'confirm_allocations',
  },
  OUTBOUND_FULFILLMENTS: {
    INITIATE: 'initiate_outbound_fulfillments',
...
/** Domain-level keys of `PERMISSION_KEYS` (e.g. `'USERS'`, `'PRODUCTS'`). */
export type PermissionDomain = keyof typeof PERMISSION_KEYS;

/** Union of every permission key string literal across all domains. */
export type PermissionKey = {
  [D in PermissionDomain]: (typeof PERMISSION_KEYS)[D][keyof (typeof PERMISSION_KEYS)[D]];
}[PermissionDomain];
