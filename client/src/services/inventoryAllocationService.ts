import type { InventoryAllocationPayload, InventoryAllocationResponse } from '@features/inventoryAllocation';
...
export const postInventoryAllocation = async (
  ...
  try {
    ...
    const endpoint = API_ENDPOINTS.INVENTORY_ALLOCATION_EXECUTE.replace(':id', orderId);
    const response = await axiosInstance.post<InventoryAllocationResponse>(endpoint, {items: items});
    return response.data;
  } catch (error) {
    console.error('Error posting inventory allocation:', error);
    throw new Error(
      'Inventory allocation failed. Please try again later.'
    );
  }
};
