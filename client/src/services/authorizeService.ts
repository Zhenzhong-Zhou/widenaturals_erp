 ...
 */
const fetchPermissions = async (): Promise<{ roleName: string; permissions: string[] }> => {
  try {
    // Fetch the raw response from the backend
    const response = await axiosInstance.get<PermissionResponse>(API_ENDPOINTS.USER_PERMISSION);
    
    // Extract and transform data to match the expected return type
    const { role_name: roleName, permissions } = response.data.data;
    
    if (!roleName || !permissions) {
      throw new Error('Invalid response format: Missing roleName or permissions.');
    }
