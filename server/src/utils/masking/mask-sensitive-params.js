...
const maskPrimitive = (val) => {
  ...
  if (BEARER_RE.test(val)) return '[REDACTED_TOKEN]';
  if (JWT_RE.test(val)) return '[REDACTED_JWT]';
  if (val.length > 32 && SECRET_RE.test(val)) return '[REDACTED_SECRET]';
  return val;
};
...
const deepMask = (value, ctx) => {
  ...
  if (Array.isArray(value)) {
    return value.map((el) =>
      deepMask(el, { depth: depth + 1, maxDepth, seen })
    );
  }
