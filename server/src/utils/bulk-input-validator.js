...
const validateBulkInputSize = (
  ...
  if (!Array.isArray(items) || items.length === 0) {
    logSystemError(`${itemType} preparation failed: Empty array received`, {
      context,
    });
  ...
  if (items.length > maxLimit) {
    logSystemInfo(
      `Bulk ${itemType} insert limit exceeded: attempted ${items.length}, max ${maxLimit}`,
      {
        context,
      }
    );
