...
const makeAudit = (row, {
  ...
  const safeFullName = (fn, ln) => {
    ...
    const parts = [fn, ln].filter(Boolean).map(s => String(s).trim()).filter(Boolean);
    return parts.join(' ') || '';
  };
  ...
  const audit = {
    ...
    createdBy,
    ...(updatedAtRaw !== null || updatedBy.firstName !== null || updatedBy.lastName !== null
  ...
  const createdName = safeFullName(audit.createdBy?.firstName, audit.createdBy?.lastName);
  const updatedName = safeFullName(audit.updatedBy?.firstName, audit.updatedBy?.lastName);
