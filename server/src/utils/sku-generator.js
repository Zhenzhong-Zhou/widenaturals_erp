...
const generateSKU = async (brandCode, categoryCode, variantCode, regionCode, lastUsedCodeMap) => {
  ...
  try {
    ...
    if (!brandCode || !categoryCode || !variantCode || !regionCode) {
      throw AppError.validationError('Missing required parameters for SKU generation.', { context });
    }
    ...
    if (lastUsedCodeMap.has(key)) {
      ...
      if (!baseCode) {
        throw AppError.validationError(`No base code found for ${key}`, { context });
      }
    ...
    logSystemError('[generateSKU] Failed to generate SKU', { ...normalized.toLog()});
    throw normalized;
  }
};
