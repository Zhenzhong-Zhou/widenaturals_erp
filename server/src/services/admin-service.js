...
const createAdmin = async ({
  ...
  try {
    ...
    // Hash the password
    const { passwordHash, passwordSalt } = await hashPasswordWithSalt(password).catch((error) => {
    ...
    const newAdmin = await createUser({
      ...
      throw new AppError('Database operation failed while creating the user.', 500, {
        type: 'DatabaseError',
        isExpected: false,
      });
    });
