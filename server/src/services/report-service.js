...
const fetchInventoryActivityLogsService = async ({ filters = {}, page = 1, limit = 20 } = {}, user) => {
  try {
    ...
    else if (scope.hasPackingMaterialAccess) permissionKey = 'packing_material';
    ...
    // Reject missing filters for scoped users
    rejectEmptyFiltersForScopedAccess(scope, filters, allowedKeys);
    
    // Dependency: warehouse
    if (scope.hasWarehouseAccess && !scope.hasFullAccess) {
      const { warehouseIds = [], productIds = [], skuIds = [], batchIds = [], packingMaterialIds = [] } = filters;
      
      if (
        warehouseIds.length === 0 &&
        (productIds.length > 0 || skuIds.length > 0 || batchIds.length > 0 || packingMaterialIds.length > 0)
      ) {
    ...
    if (scope.hasLocationAccess && !scope.hasFullAccess) {
      const { locationIds = [], productIds = [], skuIds = [], batchIds = [], packingMaterialIds = [] } = filters;
    ...
    // Scoped to packing material only
    const isPackingMaterialScopedOnly =
      scope.hasPackingMaterialAccess &&
      !scope.hasFullAccess &&
      !scope.hasProductAccess &&
      !scope.hasSkuAccess &&
    ...
    if (isPackingMaterialScopedOnly) {
      const { packingMaterialIds = [], batchIds = [] } = filters;
