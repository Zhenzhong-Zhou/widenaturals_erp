...
/**
 * Fetch full BOM composition details for a given SKU.
 *
 * This query retrieves:
 *  - Product → SKU → BOM → BOM Items → Part hierarchy
 *  - Status and user (created/updated) metadata for audit traceability
 *
 * Only active BOMs (b.is_active = TRUE) are included.
 * Each SKU is expected to have at most one active BOM at a time.
 *
 * @async
 * @function getSkuBomCompositionById
 * @param {string} skuId - UUID of the SKU to fetch BOM composition for.
 * @returns {Promise<Array<Object>>} Array of BOM and BOM item detail rows.
 * @throws {AppError} When the database query fails.
 *
 * @example
 * const rows = await getSkuBomCompositionById('374cfaad-a0ca-44dc-bfdd-19478c21f899');
 * console.log(rows.length); // e.g., 7 BOM item rows
 */
const getSkuBomCompositionById = async (skuId) => {
  const sql = `
    SELECT
      p.id AS product_id,
      p.name AS product_name,
      ...
      st_bom.name AS bom_status_name,
    ...
    LEFT JOIN boms AS b ON b.sku_id = s.id AND b.is_active = TRUE
    ...
    WHERE s.id = $1
    ORDER BY p.name ASC, s.sku ASC, b.revision DESC, bi.id ASC, pa.type ASC;
  `;
  ...
  try {
    ...
    logSystemException(error, 'Failed to fetch SKU BOM composition', {
      context: 'sku-repository/getSkuBomCompositionById',
      skuId,
    });
    
    // Wrap and rethrow as standardized AppError
    throw AppError.databaseError('Database error fetching SKU BOM composition', {
      skuId,
      hint: 'Check BOM or SKU references; ensure UUID is valid.',
    });
  }
};
