...
const { downloadFileFromS3, listBackupsFromS3 } = require('../utils/aws-s3-service');
...
const promptForFilePath = () => {
  return new Promise((resolve) => {
    ...
    rl.question('\nPlease enter the backup file path you wish to restore: ', (filePath) => {
      rl.close();
...
(async () => {
  try {
    ...
    if (!databaseName) {
      throw AppError.validationError('Database name is missing. Set DB_NAME in the environment variables.');
    }
    
    if (!dbUser) {
      throw AppError.validationError('Database user is missing. Set DB_USER in the environment variables.');
    }
    ...
    if (!localFilePath && bucketName) {
      ...
      logInfo('Available Backups:', backups.map(file => file.Key));
      logInfo('Please specify a backup file to restore from the above list or set ENCRYPTED_FILE.');
    ...
    if (localFilePath && !fs.existsSync(localFilePath)) {
      if (!bucketName) {
        throw AppError.notFoundError(`Encrypted backup file not found locally: ${localFilePath}`);
      }
      ...
      logInfo(`Encrypted file downloaded successfully from S3: ${localFilePath}`);
    }
    ...
    if (localFilePath && !fs.existsSync(localFilePath)) {
      if (!bucketName) {
        throw AppError.notFoundError(`Encrypted backup file not found locally: ${localFilePath} and no S3 bucket provided.`);
      }
    ...
    if (!localFilePath && !bucketName) {
      throw AppError.notFoundError('No backup file specified and no S3 bucket configured. Restoration cannot proceed.');
    }
    ...
    if (!localFilePath && bucketName) {
      ...
      if (backups.length === 0) {
        throw AppError.notFoundError(`No backups found in S3 bucket: ${bucketName}`);
      }
