...
(async () => {
  try {
    ...
    if (!databaseName || !encryptionKey || !dbUser || !dbPassword) {
      logError('Essential environment variables (DB_NAME, BACKUP_ENCRYPTION_KEY, DB_USER, DB_PASSWORD) are missing.');
      process.exit(1);
    }
    ...
    if (isProduction && bucketName && (!s3KeyEnc || !s3KeyIv || !s3KeySha256)) {
      ...
      try {
        ...
        console.log('Available Backups:', backups.map(file => file.Key));
      } catch (err) {
    ...
    if (!s3KeyEnc) s3KeyEnc = await promptForFilePath('Enter Encrypted Backup File Path');
    if (!s3KeyIv) s3KeyIv = await promptForFilePath('Enter IV File Path');
    if (!s3KeySha256) s3KeySha256 = await promptForFilePath('Enter SHA256 File Path');
    
    if (!s3KeyEnc || !s3KeyIv || !s3KeySha256) {
      logError('Missing required paths. Make sure to provide paths for all files.');
      process.exit(1);
    }
