...
(async () => {
  ...
  try {
    ...
    if (!warehouseRows.length || !batchRows.length || !statusRows.length || !userRows.length) {
      console.warn(`${LOG} Missing seed data — need warehouses, batch_registry, inventory_status, users.`);
      return;
    }
    ...
    try {
      ...
      try {
        ...
        info('Updated count', updated.length);
        updated.forEach((row) => {
          info(`  id=${row.id}`, `status_id=${row.status_id}, status_date=${row.status_date}`);
        });
