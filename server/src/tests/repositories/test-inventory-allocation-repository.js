...
const { getPaginatedInventoryAllocations } = require('../../repositories/inventory-allocations-repository');
...
(async () => {
  ...
  try {
    ...
    try {
      ...
      info('Total records', result.pagination.totalRecords);
      info('Row count',     result.data.length);
      const match = result.data.every(
        (row) => row.order_number.includes(testOrderNumber)
      );
