...
describe('Database Utility Tests', () => {
  ...
  describe('retry', () => {
    ...
    it('should throw an error if retries are exhausted', async () => {
      ...
      expect(logWarn).toHaveBeenCalledWith('Retry 1/2 failed: Persistent error'); // First retry warning
      expect(logWarn).toHaveBeenCalledWith('Retry 2/2 failed: Persistent error'); // Second retry warning
    });
  });
