...
const { updateProductStatusService } = require('../../services/product-service');
...
(async () => {
  ...
  try {
    ...
    if (productRows.length === 0)
      throw new AppError.notFoundError(`No product found with name: ${productName}`);
    
    const product = productRows[0];
    console.log(`${logPrefix} 📦 Found product: ${chalk.yellow(product.name)} (id: ${product.id})`);
    ...
    console.log(`${logPrefix} 🏷️ Target status: ${chalk.green(targetStatus.name)} (${targetStatus.id})`);
    ...
    logSystemException(error, 'Manual test for updateProductStatusService failed', {
      context: 'test-update-product-status',
    });
    process.exitCode = 1;
  } finally {
