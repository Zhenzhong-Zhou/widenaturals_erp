...
} = require('../../services/sku-code-base-service');
...
(async () => {
  ...
  try {
    ...
    console.log(
      `${logPrefix} 📦 Prepared brand/category pairs (includes 1 duplicate)`
    );
    console.table(
      pairs.map((p) => ({
    ...
    // ------------------------------------------------------------
    console.log(
      `${logPrefix} ▶️  Calling getOrCreateBaseCodesBulk (first run)...`
    );
