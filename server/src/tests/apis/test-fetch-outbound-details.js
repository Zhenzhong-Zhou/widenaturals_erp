...
(async () => {
  ...
  try {
    ...
    console.log(
      `${logContext} ▶️ Running getShipmentDetailsService for shipmentId=${shipmentId}...`
    );
    const result = await getShipmentDetailsService(shipmentId);

    if (!result) {
      console.warn(
        `${logContext} ⚠️ No shipment details found for shipmentId=${shipmentId}`
      );
    } else {
      console.log(
      ...
      console.log(
        `${logContext} ▶️ Fulfillments Count:`,
        result.fulfillments?.length || 0
      );
      if (result.fulfillments?.[0]) {
        console.log(
          `${logContext} ▶️ First Fulfillment Preview:`,
          result.fulfillments[0]
        );
      }
      console.log(
    ...
    console.error(
      `${logContext} ❌ Error fetching shipment details for shipmentId=${shipmentId}`
    );
