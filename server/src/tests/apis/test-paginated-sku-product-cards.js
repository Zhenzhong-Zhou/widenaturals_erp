...
(async () => {
  ...
  try {
    console.log(
      `${logContext} 🚀 Starting SKU Product-Card pagination test...\n`
    );
    ...
    console.log(
      `${logContext} 👤 Using test user: ${chalk.green(JSON.stringify(testUser))}\n`
    );
    ...
    console.log(
      `${logContext} 🔽 Sorting by: ${chalk.yellow(logicalSortKey)} → ${chalk.green(sortByColumn)}\n`
    );
    ...
    if (result.data?.length > 0) {
      ...
      console.log(
        chalk.yellow(`${logContext} ⚠ No SKU product-cards available.`)
      );
    }
