...
const { fetchBOMProductionSummaryService } = require('../../services/bom-service');
...
(async () => {
  ...
  const stepTime = (label, t0 = startTime) => {
    console.log(`${logPrefix} ⏲️ ${label}: ${((performance.now() - t0) / 1000).toFixed(3)}s`);
  };
  ...
  try {
    ...
    // ------------------------------------------------------------------------
    console.log(`${logPrefix} 🚀 Starting BOM Production Summary Service test...`);
    ...
    console.log(`${logPrefix} 📦 Parts in summary: ${chalk.green(result.parts?.length || 0)}`);
    ...
    console.log(`${logPrefix} ⏱️ Completed in ${chalk.green(`${elapsedSeconds}s`)}.`);
    stepTime('All steps completed');
    
    logSystemInfo('BOM Production Summary Service test completed successfully', {
    ...
    logSystemException(error, 'Manual test for fetchBOMProductionSummaryService failed', {
      context: 'test-fetch-bom-production-summary',
      severity: 'error',
      bomId: error?.bomId,
