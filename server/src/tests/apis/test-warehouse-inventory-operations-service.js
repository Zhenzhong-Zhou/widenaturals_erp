...
(async () => {
  ...
  try {
    ...
    if (!userRows.length || inventoryRows.length < 2 || statusRows.length < 2) {
      console.warn(`${LOG} Missing test data — need users, 2+ warehouse_inventory rows, 2+ inventory_status rows.`);
    ...
    try {
      ...
      fail('adjustQuantity — constraint violation', new Error('Should have thrown'));
      results.failed++;
    } catch (error) {
      if (
    ...
    try {
      ...
      const result = await updateWarehouseInventoryStatusService({
        warehouseId: testWarehouseId,
        updates: [
          { id: testRecords[0].id, statusId: altStatusId },
        ],
        user: testUser,
      });
    ...
    try {
      ...
      await updateWarehouseInventoryStatusService({
        warehouseId: testWarehouseId,
        updates: [
          { id: testRecords[0].id, statusId: '00000000-0000-0000-0000-000000000000' },
        ],
        user: testUser,
      });
    ...
    try {
      ...
      await adjustWarehouseInventoryQuantityService({
        warehouseId: testWarehouseId,
        updates: [{ id: testRecords[0].id, warehouseQuantity: 100, reservedQuantity: 0 }],
        user: testUser,
      });
