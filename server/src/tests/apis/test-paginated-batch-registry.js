/**
 * @fileoverview
 * Manual test script for `fetchPaginatedBatchRegistryService`.
 *
 * Purpose:
 *   - Verify batch registry pagination end-to-end
 *   - Validate ACL-driven visibility enforcement
 *   - Inspect transformed batch registry rows
 *   - Validate product vs packaging polymorphism
 *
 * Usage:
 *   node test-paginated-batch-registry.js
 */

const chalk = require('chalk');
const { pool } = require('../../database/db');
const { initStatusCache } = require('../../config/status-cache');
const { getSortMapForModule } = require('../../utils/sort-utils');
const {
  fetchPaginatedBatchRegistryService,
} = require('../../services/batch-registry-service');

(async () => {
  const client = await pool.connect();
  const logContext = chalk.cyan('[Test: Batch Registry]');
  const startTime = Date.now();
  
  try {
    // ---------------------------------------------------------
    // Init shared caches
    // ---------------------------------------------------------
    await initStatusCache();
    
    console.log(`${logContext} 🚀 Starting batch registry pagination test...\n`);
    
    // ------------------------------------------------------------
    // Step 1: Load test user (requester)
    // ------------------------------------------------------------
    const { rows } = await client.query(
      `SELECT id, role_id FROM users WHERE email = $1 LIMIT 1`,
      ['root@widenaturals.com']
      // ['jp@widenaturals.com']
    );
    
    if (rows.length === 0) {
      console.error(`${logContext} ❌ Test user not found`);
      return;
    }
    
    const { id: userId, role_id: roleId } = rows[0];
    
    const authUser = {
      id: userId,
      role: roleId,
    };
    
    console.log(
      `${logContext} 👤 Loaded auth user: ${chalk.green(
        JSON.stringify(authUser)
      )}\n`
    );
    
    // ------------------------------------------------------------
    // Step 2: Sorting setup
    // ------------------------------------------------------------
    const sortMap = getSortMapForModule('batchRegistrySortMap');
    
    const logicalSortKey = 'registeredAt'; // try: expiryDate, lotNumber
    const sortByColumn =
      sortMap[logicalSortKey] || sortMap.defaultNaturalSort;
    
    console.log(
      `${logContext} 🧮 Sorting by ${chalk.yellow(logicalSortKey)} → ${chalk.green(
        sortByColumn
      )}\n`
    );
    
    // ------------------------------------------------------------
    // Step 3: Filters & pagination
    // ------------------------------------------------------------
    const filters = {
      // batchType: 'product',
      // batchType: 'packaging_material',
      // keyword: 'gut',
    };
    
    const pagination = { page: 1, limit: 25 };
    
    console.log(`${logContext} 🔍 Filters:`);
    console.table(filters);
    console.log();
    
    // ------------------------------------------------------------
    // Step 4: Execute batch registry pagination service
    // ------------------------------------------------------------
    console.log(
      `${logContext} ▶️ Calling ${chalk.green(
        'fetchPaginatedBatchRegistryService'
      )}...\n`
    );
    
    const result = await fetchPaginatedBatchRegistryService({
      filters,
      page: pagination.page,
      limit: pagination.limit,
      sortBy: sortByColumn,
      sortOrder: 'DESC',
      user: authUser,
    });
    
    // ------------------------------------------------------------
    // Step 5: Display results
    // ------------------------------------------------------------
    const elapsedMs = Date.now() - startTime;
    const elapsedSec = (elapsedMs / 1000).toFixed(2);
    
    console.log(
      `${logContext} ✅ Completed in ${chalk.green(
        `${elapsedMs}ms`
      )} (${chalk.green(`${elapsedSec}s`)})\n`
    );
    
    console.log(`${logContext} 📄 Pagination Info:`);
    console.table(result.pagination);
    
    console.log(`\n${logContext} 🧾 First Row Preview:`);
    console.log(result.data?.[0] || chalk.yellow('No data'));
    
    console.log(`\n${logContext} 📜 Full JSON Output:`);
    console.log(chalk.gray(JSON.stringify(result, null, 2)));
    
    console.log(`\n${logContext} 📊 Batch Registry Table View`);
    
    if (result.data?.length > 0) {
      console.table(
        result.data.map((batch) => {
          // ------------------------------
          // Product batch
          // ------------------------------
          if (batch.type === 'product') {
            return {
              type: batch.type,
              lot: batch.lotNumber,
              product: batch.product?.name ?? '-',
              sku: batch.product?.sku ?? '-',
              manufacturer: batch.product?.manufacturer?.name ?? '-',
              expiry: batch.expiryDate,
              status: batch.status?.name,
              registeredBy: batch.registeredBy?.fullName,
              registeredAt: batch.registeredAt,
            };
          }
          
          // ------------------------------
          // Packaging material batch
          // ------------------------------
          return {
            type: batch.type,
            lot: batch.lotNumber,
            material: batch.packagingMaterial?.name,
            supplier: batch.packagingMaterial?.supplier?.name,
            status: batch.status?.name,
            registeredBy: batch.registeredBy?.fullName,
            registeredAt: batch.registeredAt,
            expiry: batch.expiryDate,
          };
        })
      );
    }
    
    console.log(
      `\n${logContext} 🎉 ${chalk.green(
        'Batch registry pagination test completed successfully!'
      )}`
    );
  } catch (error) {
    console.error(`${logContext} ❌ ${chalk.red(error.message)}`);
    console.error(error.stack);
  } finally {
    client.release();
    setTimeout(() => process.exit(0), 100);
  }
})();
