...
const { fetchProductBatchDetailsService } = require('../../services/product-batch-service');
...
(async () => {
  ...
  try {
    ...
    try {
      ...
      console.error(`${logContext} Unexpected error (VALID CASE):`, error.message);
    }
    ...
    try {
      await fetchProductBatchDetailsService(
        notFoundBatchId,
        testUser
      );
    } catch (error) {
