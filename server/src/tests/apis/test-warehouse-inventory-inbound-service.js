...
(async () => {
  ...
  try {
    ...
    if (
      ...
      console.warn(`${LOG} Missing seed data — need warehouses, users, 4+ unassigned batch_registry rows, inventory_status.`);
      return;
    }
    ...
    console.log(`  available batch: ${availableBatches.map((r) => r.batch_id).join(', ')}`);
    ...
    try {
      ...
      logRows.forEach((row) => {
        info(`  log=${row.id}`, `prev=${row.previous_quantity}, change=${row.quantity_change}, new=${row.new_quantity}`);
      });
      
      if (result.count !== 1) throw new Error(`Expected count=1, got ${result.count}`);
      if (logRows.length !== 1) throw new Error(`Expected 1 log entry, got ${logRows.length}`);
      if (logRows[0].previous_quantity !== 0) throw new Error('Expected previous_quantity=0');
      if (logRows[0].quantity_change !== 100) throw new Error('Expected quantity_change=100');
      if (logRows[0].new_quantity !== 100) throw new Error('Expected new_quantity=100');
    ...
    try {
      ...
      if (logRows.length !== 2) throw new Error(`Expected 2 log entries, got ${logRows.length}`);
    ...
    try {
      ...
      fail('createWarehouseInventoryService — duplicate batch', new Error('Should have thrown'));
      results.failed++;
    } catch (error) {
      if (error.message.includes('already have inventory')) {
        ...
        fail('createWarehouseInventoryService — duplicate batch rejection', error);
        results.failed++;
      }
    }
    ...
    try {
      ...
      await createWarehouseInventoryService({
        warehouseId: testWarehouseId,
        records: [
          { batchId: '00000000-0000-0000-0000-000000000000', warehouseQuantity: 50 },
        ],
        user: testUser,
      });
      
      fail('createWarehouseInventoryService — invalid batch ID', new Error('Should have thrown'));
      results.failed++;
    } catch (error) {
      if (error.message.includes('do not exist')) {
        ...
        fail('createWarehouseInventoryService — invalid batch ID rejection', error);
        results.failed++;
      }
    }
    ...
    try {
      ...
      fail('createWarehouseInventoryService — invalid quantity', new Error('Should have thrown'));
      results.failed++;
    } catch (error) {
      if (error.message.includes('positive integer')) {
        ...
        fail('createWarehouseInventoryService — invalid quantity rejection', error);
        results.failed++;
      }
    }
    ...
    try {
      ...
      if (rows[0]?.status_id !== altStatusId) {
        throw new Error(`Expected status=${altStatusId}, got ${rows[0]?.status_id}`);
      }
