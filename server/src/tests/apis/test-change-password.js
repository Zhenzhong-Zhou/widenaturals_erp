...
(async () => {
  ...
  const PASSWORDS_TO_ROTATE = Array.from(
    { length: 6 },
    (_, i) => `TEST_PASSWORD_${i + 1}`
  );
  ...
  try {
    ...
    for (const nextPassword of PASSWORDS_TO_ROTATE) {
      ...
      await changePasswordService(userId, currentPassword, nextPassword);
    ...
    await changePasswordService(userId, currentPassword, BASE_PASSWORD);
    ...
    try {
      await changePasswordService(userId, currentPassword, recentPassword);

      throw new Error('Expected password reuse rejection did not occur');
    } catch (err) {
      console.log(`${logPrefix} ✅ Correctly blocked reuse of recent password`);
    }
