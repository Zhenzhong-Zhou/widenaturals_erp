...
(async () => {
  ...
  try {
    ...
    if (!userRows.length || !logRows.length) {
      console.warn(`${LOG} Missing test data — need users and inventory_activity_log records.`);
      console.warn(`${LOG} Run test-warehouse-inventory-inbound-service.js first.`);
      return;
    }
    
    const testUser        = { id: userRows[0].id, role: userRows[0].role_id, roleName: userRows[0].role_name };
    ...
    try {
      ...
      if (result.data.length === 0) throw new Error('Expected at least one log entry');
      ...
      if (first.warehouseInventoryId === undefined) throw new Error('Missing warehouseInventoryId');
    ...
    try {
      ...
      const allMatch = result.data.every((r) => r.warehouseInventoryId === testInventoryId);
      if (!allMatch) throw new Error('Results contain entries for other inventory records');
    ...
    try {
      ...
      info('Total records', result.pagination.totalRecords);
      info('Action types', [...new Set(result.data.map((r) => r.actionTypeName))]);
    ...
    try {
      ...
      const allInRange = result.data.every((r) => {
        ...
        return t >= new Date(oneHourAgo).getTime() && t <= new Date(now).getTime();
      });
      if (!allInRange) throw new Error('Results contain entries outside the date range');
    ...
    try {
      ...
      if (result.pagination.totalRecords !== 0) throw new Error('Expected totalRecords=0');
