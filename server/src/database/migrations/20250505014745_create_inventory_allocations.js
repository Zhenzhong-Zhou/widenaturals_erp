...
exports.up = async function (knex) {
  await knex.schema.createTable('inventory_allocations', (table) => {
    ...
    table.uuid('order_item_id').nullable().references('id').inTable('order_items');
    table.uuid('transfer_order_item_id').nullable().references('id').inTable('transfer_order_items');
    ...
    table.uuid('status_id').notNullable().references('id').inTable('inventory_allocation_status');
