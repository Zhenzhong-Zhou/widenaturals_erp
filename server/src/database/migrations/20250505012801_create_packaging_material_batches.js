...
exports.up = async function (knex) {
  await knex.schema.createTable('packaging_material_batches', (table) => {
    ...
    table.uuid('packaging_material_supplier_id').notNullable().references('id').inTable('packaging_material_suppliers');
    ...
    table.index(['packaging_material_supplier_id'], 'idx_packaging_material_suppliers');
    table.index(['lot_number'], 'idx_packaging_batch_lot');
    
    table.unique(['packaging_material_supplier_id', 'lot_number'], { indexName: 'uq_material_batch_lot_per_supplier' });
  });
};
