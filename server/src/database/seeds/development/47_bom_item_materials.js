...
exports.seed = async function (knex) {
  ...
  if (Number(count) > 0) {
    console.log(
      `⏩ Skipping bom_item_materials seed — already has ${count} rows.`
    );
    return;
  }
  ...
  const bomItems = await knex('bom_items').select(
    'id',
    'part_id',
    'part_qty_per_product',
    'unit'
  );
  ...
  if (inserts.length > 0) {
    ...
    console.log(
      `Inserted ${inserts.length} bom_item_materials rows (duplicates ignored).`
    );
  } else {
