...
exports.seed = async function (knex) {
  ...
  if (!users || users.length === 0) {
    console.error('No users found in the database. Please seed the users table first.');
    return;
  }
  ...
  for (let i = 0; i < users.length; i++) {
    const { passwordHash, passwordSalt } = await hashPasswordWithSalt(passwords[i]);
    userAuthData.push({
  ...
  console.log(`${users.length} users and user_auth records seeded successfully.`);
};
