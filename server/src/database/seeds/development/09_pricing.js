...
exports.seed = async function (knex) {
  ...
  const priceTypeIds = await knex('pricing_types')
    .select('id', 'name')
    .whereIn('name', ['MSRP', 'Product Cost', 'Retail', 'Friend and Family Price', 'Discount Price']);
  ...
  productIds.forEach((product, index) => {
    priceTypeIds.forEach(priceType => {
      pricing.push({
        ...
        price: (index + 1) * 10 + Math.random().toFixed(2), // Generate random prices
  ...
  for (const price of pricing) {
    await knex('pricing')
      .insert(price)
      .onConflict(['product_id', 'price_type_id']) // Ensure uniqueness for the same product and price type
      .ignore(); // Ignore if it already exists
  }
