...
exports.seed = async function (knex) {
  ...
  await knex('warehouse_inventory')
    ...
    .ignore();
  console.log(`${warehouseInventoryEntries.length} warehouse inventory records seeded.`);
  
  // 🔹 Step 2: Insert Warehouse Inventory Lots
  const staticLotNumbers = Array.from({ length: 50 }, (_, i) => `LOT-20250${i + 1}`);
  
  const warehouseInventoryLotEntries = warehouseInventoryEntries.map((entry, i) => ({
    id: knex.raw('uuid_generate_v4()'),
  ...
  await knex('warehouse_inventory_lots')
    ...
    .ignore();
  console.log(`${warehouseInventoryLotEntries.length} warehouse inventory lot records seeded.`);
  ...
  for (const { warehouse_id, inventory_id, total_quantity } of lotQuantities) {
    ...
    const reservedQuantity = total_quantity > 0 ? Math.floor(Math.random() * (maxReserved - minReserved + 1) + minReserved) : 0;
