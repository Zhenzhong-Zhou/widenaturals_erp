...
const evaluateInventoryAllocationVisibility = async (user) => {
  ...
  try {
    ...
    const canViewAllWarehouses =
      ...
      permissions.includes(
        INVENTORY_ALLOCATION_CONSTANTS.PERMISSIONS.VIEW_ALL_ALLOCATIONS
      );
    ...
    logSystemException(
      err,
      'Failed to evaluate inventory allocation visibility',
      {
        context,
        userId: user?.id,
      }
  ...
  let candidates = batches
    .map((b) => {
      ...
      const available = Math.max(
        0,
        Number(b.warehouse_quantity || 0) - reserved
      );
...
const computeAllocationStatusPerItem = (
  ...
  const allocationMap = inventoryAllocationDetails.reduce((acc, row) => {
    acc[row.order_item_id] =
      (acc[row.order_item_id] || 0) + Number(row.allocated_quantity);
    return acc;
  }, {});
