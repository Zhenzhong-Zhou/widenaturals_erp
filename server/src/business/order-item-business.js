...
const enrichOrderItemsWithResolvedPrice = async (orderItems, client) => {
  ...
  for (const item of orderItems) {
    ...
    const priceRecord = await getPriceByIdAndSku(item.price_id, item.sku_id, client);
    if (!priceRecord) {
      throw AppError.validationError(`Invalid price_id ${item.price_id} for sku_id ${item.sku_id}`);
    }
    ...
    const metadata = (finalPrice !== dbPrice)
      ? {
